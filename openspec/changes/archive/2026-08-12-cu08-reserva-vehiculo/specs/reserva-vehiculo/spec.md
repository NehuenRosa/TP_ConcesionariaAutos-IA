...
y el vehículo a `disponible`). Si el usuario autenticado no es vendedor o
administrador, SHALL responder con error `403`; si la reserva no existe, SHALL
responder con error `404`; si la reserva no está `activa`, SHALL responder con
error `409`. La transición de estado de la reserva y del vehículo SHALL
persistirse de forma atómica.
...
- **WHEN** un vendedor cancela una reserva en estado `activa`
